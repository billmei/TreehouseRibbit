...
import com.kortaggio.ribbit.utils.MD5Util;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.userImageView = (ImageView) convertView
					.findViewById(R.id.userAvatarPhoto);
		...
		if (email.equals("")) {
			...
			Picasso.with(mContext).load(gravatarUrl)
					.placeholder(R.drawable.avatar_empty).into(holder.userImageView);
		}
		holder.nameLabel.setText(user.getUsername());

		// Display checkmark image when checked
